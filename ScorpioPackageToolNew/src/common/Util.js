...
class UtilClass {
    ...
    initDragFiles() {
        ...
        document.ondrop = (e) => {
            ...
            this.event.emit("dropFiles", files)
        }
    }

    getAndroidVersion(version) {
        let str = AndroidList[version];
        if (str == undefined) {
            return version;
        }
        return str;
    ...
    showOpenDialog(options, args, callback) {
    ...
    checkAdbEnviroment() {
        let bat = this.getAdb()
        try {
            let result = this.executeSync(`${bat} --version`, "")
    ...
    shellAndroid = async function(id, command) {
        var bat = Util.getAdb()
        return await this.executeAsync(`${bat} -s ${id} shell "${command}"`, "adb")
    }
    adbAndroid = async function(id, command) {
        var bat = Util.getAdb()
        return await this.executeAsync(`${bat} -s ${id} ${command}`, "adb")
    }
    getAndroidProp = async function(id, key) {
    ...
    getAndroidDevices = async function() {
        var bat = Util.getAdb()
